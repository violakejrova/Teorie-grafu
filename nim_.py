...
def prvnihrac(x):
      ...
      if konec() == True:
          ...
          while vyberhromadky == False:
              try:
                  hrac1h = int(input('Ze které hromádky chcete sirky odebírat?'))
                  if hrac1h > len(x) or hrac1h <= 0 or x[hrac1h-1] == 0:
                     print('Tolik hromádek ve hře není nebo je hromádka nulová')
                     print('Nápověda: Vyberte {}. hromádku a vytáhněte {} sirek.'. format(napoveda(y)[0]+1, napoveda(y)[1]))
                     vyberhromadky = False
                  else:
                      vyberhromadky = True                  
              except ValueError:
                 print('zvolte číselnou hodnotu') 
                 print('Nápověda: Vyberte {}. hromádku '. format(napoveda(y)[0]+1))
                 vyberhromadky = False
          
                 
          vybersirek = False
          
          while vybersirek == False:
              try:
                hrac1s = int (input('Kolik sirek chcete odebrat?'))
                if hrac1s <= 0:
                    print('Musíte vytáhnout alespoň jednu sirku.') 
                    print('Nápověda: Vyberte {} sirek.'. format(napoveda(y)[1]))
                    vybersirek = False
                    hrac1s = int(input('Kolik sirek chcete tahat?'))
  
                elif hrac1s > x[hrac1h-1]:
                    print('Tolik sirek ve', hrac1h, '. hromadce není.')
                    print('Nápověda: Vyberte {} sirek.'. format (napoveda(y)[1]))
                    vybersirek = False
                    hrac1s = int(input('Zvolte počet sirek:'))
                vybersirek = True
              except ValueError:
                  print('zvolte číselnou hodnotu') 
                  print('Nápověda: Vyberte {} sirek.'. format (napoveda(y)[1]))
              ...
              if vyberhromadky == True and vybersirek == True:
